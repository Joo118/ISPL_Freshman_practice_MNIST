...
class Classifier(object):
    ...
    def build(self, x, reuse=None):
        ...
        with tf.variable_scope('model', reuse=reuse):
            x = tf.layers.conv2d(x, 32, 5, padding='same', activation=tf.nn.relu, name='conv_1')
            x = tf.layers.max_pooling2d(x, pool_size=2, strides=2, padding='same', name='max_pool_1')
            x = tf.layers.conv2d(x, 64, 5, padding='same', activation=tf.nn.relu, name='conv_2')
            x = tf.layers.max_pooling2d(x, pool_size=2, strides=2, padding='same', name='max_pool_2')
            x = tf.reshape(x, [-1, 7 * 7 * 64], name='vectorized')
            x = tf.layers.dense(x, 1024, name='FC_1')
            x = tf.layers.dropout(x, rate=0.5, name='dropout1')
            x = tf.layers.dense(x, 10, name='FC_2')
    ...
    def train(self):
        ...
        val_logit = self.build(val_img, True) # True였음
